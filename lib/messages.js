...
class Message extends require('./modelbase') {
  ...
  addRecipients(recipients) {
    ...
    if (!recipients || !recipients.length || !Array.isArray(recipients)) {
      return new Promise((resolve, reject) => {
        reject(new Error('Recipients must be an array of emails/usernames'));
      });
    }
  ...
  static getMessages(query) {
    return knex
      ...
      .andWhere('messages.private', '!=', 'TRUE')
