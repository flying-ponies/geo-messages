...
class Message extends require('./modelbase') {
  ...
  save() {
    ...
    return new Promise((resolve, reject) => {
      this.validates()
        .then(() => {
          return st.geomFromText(this.fields.location, 4326);
        })
        .then((geom) => {
          fields.location = geom;
  ...
  static findInRange(pos, range) {
    ...
    // const query = `ST_DWithin(ST_GeomFromText('${pos}', 4326)::geography, location, ${range})`;
    return knex
      .select(
        'messages.id',
        'messages.title',
