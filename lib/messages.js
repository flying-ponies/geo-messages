...
class Message extends require('./modelbase') {
  ...
  getRecipients() {
    ...
    return knex
      ...
      .leftJoin('users', 'message_recipients.user_id', '=', 'users.id')
      .where('message_recipients.message_id', this.fields.id);
  }
  ...
  removeRecipient(recipient) {
    ...
    if (!recipient) {
      return new Promise((resolve, reject) => {
        reject(new Error('Recipient must be passed as an argument'));
      });
    }
    ...
    return knex.first('id')
      ...
      .orWhere(knex.raw('LOWER(email)'), recipient)
      .then((row) => {
        if (row && row.id !== this.fields.user_id) {
          return knex('message_recipients')
            .where('message_id', this.fields.id)
            .andWhere('user_id', row.id)
            .del();
        } else {
          return new Promise((resolve, reject) => {
