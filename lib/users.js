...
class User extends require('./modelbase') {
  ...
  static search(username) {
    return knex
      .select('username')
      .from('users')
      .where(knex.raw('LOWER(username)'), 'like', `${username.toLowerCase()}%`)
      .limit(5)
      .then((rows) => {
        return new Promise((resolve, reject) => {
  ...
  findInRange(pos, range) {
    ...
    let query = `ST_DWithin(${geomPos}::geography, location, ${range}) AND (messages.private != TRUE OR (messages.private AND messages.id IN (SELECT message_id FROM message_recipients WHERE user_id = ${this.fields.id})))`;
    return Message.getMessages(query);
  }
}
